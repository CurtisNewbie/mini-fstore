...
func MigrateFileServer(c common.ExecContext) error {
	...
	c.Log.Infof("Preparing to migrate from file-server, dry-run: %v", dryrun)
	...
	c.Log.Infof("Connecting to file-server's database (%s:%s/%s)", host, port, db)
	...
	c.Log.Infof("File-server's database (%s:%s/%s) connected", host, port, db)
	defer func() {
		d, err := fsconn.DB()
		if err != nil {
			return
		...
		d.Close()
		c.Log.Infof("File-server's database (%s:%s/%s) disconnected", host, port, db)
	}()
	...
	c.Log.Debugf("Desc file_info: %+v", fileInfoCols)
	...
	if !hasFstoreFileId {
		c.Log.Errorf("Table file_info doesn't have column %v, please run the following SQL before migration\n\n%s\n", FILE_ID_COL, FILE_INFO_MIGR_SQL)
	...
	c.Log.Infof("Start migrating file-server's file_info to mini-fstore's file, file-server base path: '%v'", basePath)
	...
	for {
		...
		for _, f := range fileInfos {
			...
			if em := migrateFileInfo(c, fsconn, f, basePath, dryrun); em != nil {
	...
	c.Log.Info("Finished migrating file-server's file_info to mini-fstore's file")

	return nil
}

// Migrate FileInfo to mini-fstore
func migrateFileInfo(c common.ExecContext, fsconn *gorm.DB, fi FileInfo, basePath string, dryrun bool) error {
	path := fileServerPath(fi, basePath)

	f, eo := os.Open(path)
	if eo != nil {
		if os.IsNotExist(eo) {
			c.Log.Warnf("File not exists, skipped, uuid: %v, path: %v, %v", fi.Uuid, path, eo)
			return nil
		}
		return eo
	}
	...
	if dryrun {
		...
		c.Log.Infof("Will copy file '%s' to '%s'", path, storage)
		return nil
	}

	fileId, eu := UploadFile(c, f, fi.Name)
	if eu != nil {
		return eu
	}

	c.Log.Infof("Uploaded file '%s' to mini-fstore, updating file_info, uuid: %s, fileId: %s", path, fi.Uuid, fileId)
